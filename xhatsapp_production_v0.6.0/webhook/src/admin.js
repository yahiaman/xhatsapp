...
<body><main class="wrap">
  ...
  <h2>Dictionnaire des agences interdites</h2><p class="muted">Toute citation d'une agence de cette liste dans un groupe surveillé est immédiatement supprimée pour tous, avec rappel de neutralité dans le groupe et notification d'information dans Groupe_admin.</p>
...
const tokenInput=document.getElementById('token'), login=document.getElementById('login'), panel=document.getElementById('panel'), statusBox=document.getElementById('status'), tbody=document.getElementById('groups'), scheduleBody=document.getElementById('schedules'), agencyBody=document.getElementById('agencies'), agencyInput=document.getElementById('new-agency'), agencySearch=document.getElementById('search-agency');
...
function renderAgencies(items){agencyBody.replaceChildren();const filter=agencySearch.value.trim().toLowerCase();const filtered=items.filter(a=>a.name.toLowerCase().includes(filter));for(const agency of filtered){const row=document.createElement('tr');const name=document.createElement('td');name.textContent=agency.name;const date=document.createElement('td');date.textContent=agency.created_at?new Date(agency.created_at).toLocaleDateString('fr-FR'):'-';const action=document.createElement('td');const delBtn=document.createElement('button');delBtn.className='danger';delBtn.textContent='Supprimer';delBtn.onclick=async()=>{if(!confirm('Supprimer l’agence « '+agency.name+' » du dictionnaire ?'))return;try{await api('/admin/api/agencies/'+encodeURIComponent(agency.id),{method:'DELETE'});status('Agence '+agency.name+' supprimée');await loadAgencies();}catch(err){status(err.message,true);}};action.appendChild(delBtn);row.append(name,date,action);agencyBody.appendChild(row);}}
...
document.getElementById('add-agency').onclick=async()=>{const name=agencyInput.value.trim();if(!name)return;try{await api('/admin/api/agencies',{method:'POST',body:JSON.stringify({name})});agencyInput.value='';status('Agence « '+name+' » ajoutée');await loadAgencies();}catch(err){status(err.message,true);}};
